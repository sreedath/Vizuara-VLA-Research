#!/usr/bin/env python3
"""Experiment 165: Embedding norm analysis for OOD detection.

Tests whether embedding vector norms (L2, L1, Linf) carry OOD signal
independently of direction (cosine distance). Compares norm-based vs
direction-based detection.
"""

import json, os, sys, datetime
import numpy as np
import torch
from pathlib import Path
from PIL import Image, ImageFilter

SCRIPT_DIR = Path(__file__).parent
REPO_DIR = SCRIPT_DIR.parent
EXPERIMENTS_DIR = REPO_DIR / "experiments"
EXPERIMENTS_DIR.mkdir(exist_ok=True)
RESULTS_DIR = str(EXPERIMENTS_DIR)

SIZE = (256, 256)
rng = np.random.RandomState(42)

def create_highway(idx):
    img = np.zeros((*SIZE, 3), dtype=np.uint8)
    img[:SIZE[0]//2] = [135, 206, 235]; img[SIZE[0]//2:] = [80, 80, 80]
    img[SIZE[0]//2:, SIZE[1]//2-3:SIZE[1]//2+3] = [255, 255, 255]
    return np.clip(img.astype(np.int16) + rng.randint(-5, 6, img.shape).astype(np.int16), 0, 255).astype(np.uint8)

def create_urban(idx):
    img = np.zeros((*SIZE, 3), dtype=np.uint8)
    img[:SIZE[0]//3] = [135, 206, 235]; img[SIZE[0]//3:SIZE[0]//2] = [139, 119, 101]; img[SIZE[0]//2:] = [60, 60, 60]
    return np.clip(img.astype(np.int16) + rng.randint(-5, 6, img.shape).astype(np.int16), 0, 255).astype(np.uint8)

def create_rural(idx):
    img = np.zeros((*SIZE, 3), dtype=np.uint8)
    img[:SIZE[0]//3] = [100, 180, 255]; img[SIZE[0]//3:SIZE[0]*2//3] = [34, 139, 34]; img[SIZE[0]*2//3:] = [90, 90, 90]
    return np.clip(img.astype(np.int16) + rng.randint(-8, 9, img.shape).astype(np.int16), 0, 255).astype(np.uint8)

def apply_fog(a, alpha):
    return np.clip(a*(1-alpha)+np.full_like(a,[200,200,210])*alpha, 0, 255).astype(np.uint8)
def apply_night(a): return np.clip(a*0.15, 0, 255).astype(np.uint8)
def apply_blur(a, r=8): return np.array(Image.fromarray(a).filter(ImageFilter.GaussianBlur(radius=r)))
def apply_noise(a, s=50): return np.clip(a.astype(np.float32)+np.random.normal(0,s,a.shape), 0, 255).astype(np.uint8)
def apply_occlusion(a):
    o=a.copy(); h,w=o.shape[:2]; o[h//4:3*h//4, w//4:3*w//4]=128; return o

def extract_hidden(model, processor, image, prompt, layers):
    inputs = processor(prompt, image).to(model.device, dtype=torch.bfloat16)
    with torch.no_grad():
        fwd = model(**inputs, output_hidden_states=True)
    return {l: fwd.hidden_states[l][0, -1, :].float().cpu().numpy() for l in layers}

def cosine_distance(a, b):
    return float(1 - np.dot(a, b) / (np.linalg.norm(a) * np.linalg.norm(b) + 1e-10))

def compute_auroc(id_scores, ood_scores):
    id_scores = np.asarray(id_scores)
    ood_scores = np.asarray(ood_scores)
    n_id, n_ood = len(id_scores), len(ood_scores)
    if n_id == 0 or n_ood == 0: return 0.5
    count = sum(float(np.sum(o > id_scores) + 0.5 * np.sum(o == id_scores)) for o in ood_scores)
    return count / (n_id * n_ood)

def main():
    print("=" * 60)
    print("Experiment 165: Embedding Norm Analysis")
    print("=" * 60, flush=True)

    from transformers import AutoModelForVision2Seq, AutoProcessor
    print("Loading OpenVLA-7B...", flush=True)
    model = AutoModelForVision2Seq.from_pretrained(
        "openvla/openvla-7b", torch_dtype=torch.bfloat16,
        device_map="auto", trust_remote_code=True)
    processor = AutoProcessor.from_pretrained("openvla/openvla-7b", trust_remote_code=True)
    model.eval()

    prompt = "In: What action should the robot take to drive forward at 25 m/s safely?\nOut:"
    layers = [3, 32]

    creators = [create_highway, create_urban, create_rural]
    n_cal = 8; n_test = 6

    # Extract embeddings
    cal_arrs = [creators[i%3](i) for i in range(n_cal)]
    test_arrs = [creators[(i+n_cal)%3](i+n_cal) for i in range(n_test)]

    cal_embs = {l: [] for l in layers}
    for arr in cal_arrs:
        h = extract_hidden(model, processor, Image.fromarray(arr), prompt, layers)
        for l in layers:
            cal_embs[l].append(h[l])

    id_embs = {l: [] for l in layers}
    for arr in test_arrs:
        h = extract_hidden(model, processor, Image.fromarray(arr), prompt, layers)
        for l in layers:
            id_embs[l].append(h[l])

    ood_transforms = {
        "fog_30": lambda a: apply_fog(a, 0.3),
        "fog_60": lambda a: apply_fog(a, 0.6),
        "night": apply_night,
        "blur": apply_blur,
        "noise": apply_noise,
        "occlusion": apply_occlusion,
    }
    ood_embs = {l: [] for l in layers}
    for cat, tfn in ood_transforms.items():
        for j in range(5):
            arr = tfn(test_arrs[j % n_test])
            h = extract_hidden(model, processor, Image.fromarray(arr), prompt, layers)
            for l in layers:
                ood_embs[l].append(h[l])

    # Compute different score types
    results = {}
    for l in layers:
        centroid = np.array(cal_embs[l]).mean(axis=0)
        cal_norms = {"l2": [], "l1": [], "linf": [], "mean_act": []}
        for e in cal_embs[l]:
            cal_norms["l2"].append(np.linalg.norm(e))
            cal_norms["l1"].append(np.sum(np.abs(e)))
            cal_norms["linf"].append(np.max(np.abs(e)))
            cal_norms["mean_act"].append(np.mean(np.abs(e)))

        metrics = {
            "cosine_dist": lambda e: cosine_distance(e, centroid),
            "euclidean_dist": lambda e: float(np.linalg.norm(e - centroid)),
            "l2_norm": lambda e: float(np.linalg.norm(e)),
            "l1_norm": lambda e: float(np.sum(np.abs(e))),
            "linf_norm": lambda e: float(np.max(np.abs(e))),
            "mean_activation": lambda e: float(np.mean(np.abs(e))),
            "norm_deviation": lambda e: float(abs(np.linalg.norm(e) - np.mean(cal_norms["l2"]))),
            "sparsity": lambda e: float(np.sum(np.abs(e) < 0.01) / len(e)),
        }

        layer_results = {}
        for mname, mfn in metrics.items():
            id_scores = [mfn(e) for e in id_embs[l]]
            ood_scores = [mfn(e) for e in ood_embs[l]]
            auroc = compute_auroc(np.array(id_scores), np.array(ood_scores))
            # Check if lower score = more OOD (flip AUROC)
            auroc_flip = 1.0 - auroc
            best_auroc = max(auroc, auroc_flip)
            direction = "higher=OOD" if auroc >= auroc_flip else "lower=OOD"

            layer_results[mname] = {
                "auroc": best_auroc,
                "direction": direction,
                "id_mean": float(np.mean(id_scores)),
                "id_std": float(np.std(id_scores)),
                "ood_mean": float(np.mean(ood_scores)),
                "ood_std": float(np.std(ood_scores)),
            }
            print(f"  L{l} {mname:>18s}: AUROC={best_auroc:.4f} ({direction}) "
                  f"ID={np.mean(id_scores):.4f}±{np.std(id_scores):.4f} "
                  f"OOD={np.mean(ood_scores):.4f}±{np.std(ood_scores):.4f}", flush=True)

        results[f"L{l}"] = layer_results

    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    output = {
        "experiment": "norm_analysis",
        "experiment_number": 165,
        "timestamp": ts,
        "n_cal": n_cal, "n_test_id": n_test,
        "layers": layers,
        "results": results,
    }
    path = os.path.join(RESULTS_DIR, f"norm_analysis_{ts}.json")
    with open(path, "w") as f:
        json.dump(output, f, indent=2)
    print(f"\nSaved: {path}")

if __name__ == "__main__":
    main()
